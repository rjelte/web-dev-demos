	...
	angular.module('main.controllers', [])
		.controller('MainCtrl', function($scope, $log, $sce, $http, $state, $timeout) {
			$http.get('/api/quotes/').success(function(data){
				$scope.quotes = data;
				$scope.quote = $sce.trustAsHtml(data[Math.floor(Math.random() * $scope.quotes.length)].text);
			});
		...
		controller('KauntoCtrl', function($scope, $log, $document) {
			$log.debug("Kaunto!");
			$scope.symbols = [" ", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十"];
			$scope.s = {level:0};
			$scope.numSquares = 16;
			$scope.squares = [];
			_.each(_.range($scope.numSquares), function(num){
				$scope.squares.push({level:0});
			});
			...
			$scope.initializeBoard = function(){
				...
				while(square1 == square2){
					square2 = Math.floor(Math.random() * 16);
				}
				$scope.squares[square1].level = 1;
				$scope.squares[square2].level = 2;
			};
			$scope.initializeBoard();

			$scope.isMatch = function(a, b){
				return ($scope.squares[a].level == $scope.squares[b].level);
			};
			//  0  4  8 12
			//  1  5  9 13
			//  2  6 10 14
			...
			$scope.getSymbol = function(){
				return {level:$scope.rand(1,2)};
			};

			$scope.nope = function(dir){
				$log.debug("No moves to the ", dir);
			};
			...
			$scope.slideLeft = function(){
				...
				for(var x = 0; x < 15; x++){
					var thisSquare = $scope.squares[i].level;
					if (thisSquare > 0){
						var myPos = i;
						while(myPos > 3){
							var nextSquare = $scope.squares[myPos - rowDiff].level;
							if(nextSquare === 0){
								$scope.squares[myPos - rowDiff].level = $scope.squares[myPos].level;
								$scope.squares[myPos].level = 0;
								moved = true;
							}
							if(nextSquare == thisSquare){
								$scope.squares[myPos - rowDiff].level++;
								$scope.squares[myPos].level = 0;
								moved = true;
							}
							myPos -= rowDiff;
				...
				if (moved){
					var newSquare = Math.floor(Math.random() * 5) + 11;
					while($scope.squares[newSquare].level !== 0){
						newSquare = Math.floor(Math.random() * 5) + 11;
					}
					$log.debug(newSquare);
					$scope.squares[newSquare] = $scope.getSymbol();
			...
			$scope.slideRight = function(){
				...
				if (moved){
					var newSquare = Math.floor(Math.random() * 5);
					while($scope.squares[newSquare].level !== 0){
						newSquare = Math.floor(Math.random() * 5);
					}
					$log.debug(newSquare);
					$scope.squares[newSquare] = $scope.getSymbol();
					$log.debug($scope.squares);
			...
			$scope.slideUp = function(){
				...
				if (moved){
					var newSquare = (Math.floor(Math.random() * 4) + 1) * 4- 1;
					$log.debug(newSquare);
					while($scope.squares[newSquare].level !== 0){
						newSquare = (Math.floor(Math.random() * 4) + 1) * 4 - 1;
					}
					$log.debug(newSquare);
					$scope.squares[newSquare] = $scope.getSymbol();
					$scope.$apply();
			...
			$scope.slideDown = function(){
				...
				if (moved){
					var newSquare = Math.floor(Math.random() * 4) * 4;
					while($scope.squares[newSquare].level !== 0){
						newSquare = Math.floor(Math.random() * 4) * 4;
					}
					$log.debug(newSquare);
					$scope.squares[newSquare] = $scope.getSymbol();
					$log.debug($scope.squares);
			...
			$document.on('keyup', $scope.keyInput);
				$scope.$on('$destroy', function () {
					$document.off('keyup', $scope.keyInput);
				});
			...
			// console.log(pali("hannah"));
			// console.log(pali("timothy"));
